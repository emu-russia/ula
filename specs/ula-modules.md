...
- `GD` — защёлка с инверсным разрешением (`nE`):
  ...
  полный разбор всех триггероподобных блоков (типы `GD`/`FD`/`TCE`/`TRCE`/`TRC`/`SR`,
  инвентаризация 65 запоминающих кластеров и вырезки из аннотированного нетлиста) —
  в [seq.md](/specs/seq.md) (задача #7);
...
Поимённая таблица **всех внутренних сигналов** (название → откуда → куда →
описание) — в [ula-signals.md](ula-signals.md) (задача
...
`nCLK7` переключается один раз за период `OSC`
...
- `g518`: `CLKHC6 = nor(nTCLKA, nC[5])` — такт вертикального счётчика (см. `vcounter`);
...
по цветовым каналам (перестановка описана ниже в этом разделе).
...
Полный посигнальный список проводки верхнего уровня (откуда/куда/описание) —
в [ula-signals.md](ula-signals.md).
